# train_F7_F8.py
"""EQL - entrenamiento F7..F8 (sin, cos, id, multiply, sigmoid), capas v=[3,3,2] + eval finita."""
import os, math, sys
...
default_params = {
    ...
    'id': 301,
    'train_val_file': 'data/F7data_train_val',   # puedes pasar F8 por CLI
    'test_file':      'data/F7data_test',
    'epoch_factor': 1000,
    'num_h_layers': 3,
    'layer_width': [3, 3, 2],   # v = [3,3,2]
...
class Model(object):
    def __init__(self, mode, layer_width, num_h_layers, reg_sched, output_bound,
        ...
        assert len(self.layer_widths) == num_h_layers
        ...
        # Incluye sigmoid para ayudar en F7/F8 (no hay divisiones)
        self.eql_layers = []
        for lw in self.layer_widths:
            self.eql_layers.append(
                eql.EQL_Layer(sin=lw, cos=lw, id=lw, multiply=lw, sigmoid=lw,
...
if __name__ == '__main__':
    ...
    for ep in range(1, max_episode + 1):
        ...
        penalty_flag = True;  eqlearner.train(input_fn=penalty_train_input)
        penalty_flag = False; eqlearner.train(input_fn=train_input, hooks=[logging_hook])
    ...
    results["equation"] = "Ver latex_y*.png / graph_y*.png en la carpeta del experimento."
    ...
    save_summary_csv_row(os.path.join(outdir, "..", "all_runs_summary.csv"), csv_row, header_order=list(csv_row.keys()))
